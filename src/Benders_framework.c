...
void Benders_framework(void)
{
	...
	if (CPXsetusercutcallbackfunc(env, mycutcallback, &cutinfo))
		goto TERMINATE;
	if (CPXsetlazyconstraintcallbackfunc(env, mycutcallback, &cutinfo))
		goto TERMINATE;
	if (CPXsetheuristiccallbackfunc(env, Heur, NULL))
		goto TERMINATE;
	if (set_mip_start(env, lp, cur_numcols))
		goto TERMINATE;
...
{ // Mode 0- you're consulting after solving the complete and final MIP, 1-You're consulting after solving an intermediate MIP.
	int status, i;
	clock_t end;
	double cputime;
	FILE *out;
...
{ // Here we will set the branch and cut parameters.
	int acumstatus = 0;
...
int CPXPUBLIC mycutcallback(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p)
{
	...
	*useraction_p = CPX_CALLBACK_DEFAULT;
	...
	if (flag_solve_SP == 1)
	{
		...
		if (vers != 3 && Flag_Integer == 1)
		{ // modifiedy by ivan 5172019
			Define_Core_Point();
		...
		for (i = 0; i < NN - 1; i++)
		{ // Solve (i,j) primal/dual subproblems
			for (j = i + 1; j < NN; j++)
			{
...
void Define_Core_Point(void)
{
	...
	if (w_p_median_constr == 1)
	{
		...
		{
			if (eli_per_com[i] > 1)
			{ // If there is more than one candidate then you can do this
				if (fixed_zero[i] == 0)
				{
					for (k = 0; k < NN; k++)
					{
			...
			{ // What to do in the event that there is only one potential assignment.
				for (k = 0; k < NN; k++)
				{
	...
	else
	{
		...
		{ // This is when there is only one candidate hub
			for (i = 0; i < NN; i++)
			{
...
void Check_CP_MW(double *z_sol, int i, int j)
{
	...
	for (h = 0; h < count_cand_hubs; h++)
	{
		...
		if (ABS(temp) > 0.00001)
		{
			...
			values_hub[h] = 0;
		acum += values_hub[h];
		if (values_hub[h] > sum_core)
			sum_core = values_hub[h];
	}
	if (acum < 0.001)
		sum_core = 0;
	else
...
double solve_as_LP(CPXENVptr env, CPXLPptr lp)
{
	...
	if (status)
		fprintf(stderr, "Failed to getx LP.\n");
	end = clock();
	cputime = (double)(end - start) / CLOCKS_PER_SEC;
	...
	if (vers != 5)
	{
		for (i = 0; i < count_cand_hubs; i++)
		{
			if (value + dj[pos_z[cand_hubs[i]][cand_hubs[i]]] > UpperBound + 0.01)
			{
				...
				if (status)
					fprintf(stderr, "CPXchange bounds failed.\n");
			...
			for (k = 0; k < NN; k++)
			{
				if (i != k && fixed_zero[k] == 0)
				{
					...
					if (ABS(coeff_z) > 0.00001)
					{
						...
						initial_cuts[0].origval[pos_z[i][k]] = 0;
				}
			}
		}
	}
...
{ // Need to confirm if we still need to send aguments like the count_fixed with a &
	int flag_added = 0;
	if (flag_fix_to_zero)
		flag_added += pe_fix_to_zero(env, lp, x, value, dj, count_fixed);

	if (flag_fix_to_one)
		flag_added += pe_fix_to_one(env, lp, x, value, dj, count_fixed);
